...
- `created_by_user_id` / `updated_by_user_id` で誰が作成・更新したかを追跡。複数スタッフが触る前提のため
- 修正履歴は papertrail 等の gem で別テーブルに保持する（具体的なライブラリは技術スタック選定で決定）
...
1. **ログイン画面** — メール + パスワード（または Magic Link）
