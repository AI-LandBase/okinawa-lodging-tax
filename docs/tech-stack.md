...
> ロゴ・住所等は §1規律1 通り Rails credentials（テキスト）と `app/assets/images/facility/*`（バイナリ）から読み込む（[v0要件 §1規律1](./v0-requirements.md#規律1-ユーザー固有データはコードに書かない) / [decisions.md 2026-04-07](./decisions.md) 参照）。
...
│   ├── assets/
│   │   └── images/
│   │       └── facility/        # ロゴ等バイナリ（.gitignore 対象）
│   ├── controllers/
│   ├── models/        # Stay, Receipt, MonthlyExport, User
...
├── config/
│   ├── credentials.yml.enc      # 施設情報・チャネル等（暗号化、コミット対象）
│   └── master.key               # 復号鍵（.gitignore 対象、Kamal secrets で配布）
├── db/
│   ├── migrate/
│   └── seeds.rb                 # 初期スタッフ作成
├── docs/
...
- **税率・条例改定への対応**:
  - スキーマ変更ではなく **`app/services/tax_rules/okinawa.rb` の `VERSION` 更新**で吸収する
