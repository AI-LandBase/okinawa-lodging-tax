...
.PHONY: crm-bash
crm-bash: ## Sales CRM コンテナに入る
	docker compose -f compose.development.yaml --env-file .env.development exec sales_crm bash

.PHONY: crm-test
crm-test: ## Sales CRM の RSpec テストを実行
	docker compose -f compose.development.yaml --env-file .env.development exec \
		-e DATABASE_URL=postgres://postgres-user:postgres-password@postgresdb:5432/sales_crm_test \
		sales_crm bundle exec rspec

.PHONY: crm-logs
crm-logs: ## Sales CRM のログを表示
	docker compose -f compose.development.yaml --env-file .env.development logs -f sales_crm
