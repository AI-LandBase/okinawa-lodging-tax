...
- **将来の配布モデル**: 沖縄北部の小規模宿泊事業者の端末保持率の低さが地域課題のため、**県の宿泊税関連の助成金制度（制度名・要件は今後確認）を活用した Mac / iPad / Square プリインストール済み端末キット販売** を志向する（[`docs/decisions.md` 2026-04-08](./docs/decisions.md) 参照）

### 動作前提デバイス

- **アプリの動作要件**: モダンブラウザ（Chrome / Safari / Edge 最新版）が動作する任意の PC。Docker 運用のため OS 非依存。**推奨は Mac**（販売キット同梱のため動作検証・サポート窓口を集約）
- **iPad**（オプション）: iPad Safari での現場入力に対応。採用しない事業者は紙台帳運用で完結できる
- **Square**（決済端末として前提）: v0 は API 連携なし・スタッフ手入力。Stay モデルに `payment_method` カラムで備えのみ持つ。API 連携は v1
- **運用形態**: 紙台帳ベース運用（ベースライン） / iPad 現場入力運用（オプション）のどちらも選べる
...
施設名・住所・登録番号・チャネル名・領収書ロゴ等はコードに直書きしない。

> **注**: 当初は `config/facility.json` への集約を想定していたが、[`docs/decisions.md` 2026-04-07](./docs/decisions.md) で方針変更済み。v0 ではテキスト設定を **Rails credentials**、バイナリ資産を **`app/assets/images/facility/*`** に分離して管理する。
